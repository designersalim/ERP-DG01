  ...
  const synchronizedJobCards = useMemo(() => {
    return bookings.map(b => {
      ...
      const virtualCard: JobCard = {
        id: `jc-virtual-${b.id}`,
        bookingId: b.id,
        jobCardNo: `ATC-JC-${b.ref || 'WO'}-${b.styleNumber || 'STYLE'}`,
    ...
    <div className="space-y-6">
                          ...
                          <td className="py-3.5 px-4 text-right">
                            <div className="flex items-center justify-end gap-1.5 flex-wrap">
                              ...
                              <button
                                ...
                                className="p-1 px-2 bg-neutral-900 hover:bg-neutral-950 text-white font-bold uppercase text-[9.5px] rounded flex items-center gap-0.5 cursor-pointer transition-colors shadow-xs"
                              >
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl max-w-4xl w-full shadow-lg p-6 space-y-4 text-xs">
            <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
              <h3 className="text-sm font-black text-slate-800 uppercase flex items-center gap-1.5 font-sans">
                <ClipboardList className="w-4 h-4 text-emerald-600" /> Job Card Print Preview (জব কার্ড প্রিন্ট প্রিভিউ)
              </h3>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => triggerPrintJobCard(activePreview)}
                  className="px-4 py-1.5 bg-neutral-900 hover:bg-neutral-950 text-white font-extrabold text-[10px] uppercase tracking-wider rounded-lg flex items-center gap-1 cursor-pointer transition-all"
                >
            ...
            <div className="flex justify-center bg-white p-6 border border-slate-250 rounded-xl max-h-[500px] overflow-y-auto">
              <div 
                className="w-[195mm] min-h-[260mm] p-[10mm] border border-neutral-300 shadow-sm font-sans bg-white text-neutral-900 relative box-border flex flex-col justify-between" 
              >
                {/* Visual Header */}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-[10.5px] bg-neutral-50 p-3.5 border border-neutral-200 rounded-lg mt-3">
                  <div>
                    <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Card ID:</span>
                    <span className="font-extrabold text-neutral-900">{activePreview.jobCardNo}</span>
                  </div>
                  <div>
                    <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Buyer Name:</span>
                    <span className="font-extrabold text-neutral-900">{activePreview.buyerName}</span>
                  </div>
                  <div>
                    <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Garments Factory:</span>
                    <span className="font-extrabold text-neutral-900">{activePreview.factoryName}</span>
                  </div>
                  <div>
                    <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">PO Number:</span>
                    <span className="font-bold text-neutral-800">{activePreview.poNumber || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Style / Brand:</span>
                    <span className="font-bold text-neutral-800">{activePreview.styleNumber || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Target Date:</span>
                    <span className="font-extrabold text-slate-900">{activePreview.targetDate || 'N/A'}</span>
                  </div>
                </div>

                {/* Materials specs */}
                <div className="border border-neutral-200 rounded-lg overflow-hidden mt-3 text-[10.5px]">
                  <div className="bg-neutral-800 text-white px-3 py-1.5 font-bold uppercase tracking-wider flex justify-between">
                    <span>Core Accessory Specifications / পণ্যের বিবরণ</span>
                    <span>Status: {activePreview.status.toUpperCase()}</span>
                  </div>
                  <div className="p-3 space-y-3">
                    <div className="grid grid-cols-2 gap-3 border-b border-neutral-100 pb-2">
                      <div>
                        <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Accessory Item Part Name:</span>
                        <span className="font-extrabold text-neutral-900 text-xs">{activePreview.itemName}</span>
                      </div>
                      <div>
                        <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Assigned Machine No:</span>
                        <span className="font-bold text-emerald-800 uppercase">[ {activePreview.machineNo || 'M/C Division'} ]</span>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Yarn / Materials Combination:</span>
                        <p className="font-semibold text-neutral-700 leading-normal">{activePreview.materialDetails || 'Raw standard yarn/polyester.'}</p>
                      </div>
                      <div>
                        <span className="block text-[8px] uppercase font-bold text-slate-400 font-bold">Dyeing & Color Standards:</span>
                ...
                {/* Sizes list */}
                <div className="border border-neutral-200 rounded-lg overflow-hidden mt-3 text-[10.5px]">
                  <div className="bg-neutral-100 px-3 py-1.5 font-extrabold text-neutral-800 uppercase tracking-wider">
                    SizeWise Targets (সাইজ অনুযায়ী উৎপাদনের পরিমাণ)
                  </div>
                  <table className="w-full text-left text-xs text-neutral-700">
                    <thead className="bg-neutral-50 border-b border-neutral-200 font-bold uppercase text-[8px] text-slate-400">
                      <tr>
                        <th className="py-1 px-3">Size Name</th>
                        <th className="py-1 px-3 text-right">Target Qty</th>
                        <th className="py-1 px-3 text-right font-mono">Actual Qty Check</th>
                        <th className="py-1 px-3 text-center">Defects Check</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-155">
                      {activePreview.sizeWise && activePreview.sizes.length > 0 ? (
                        activePreview.sizes.map(s => (
                          <tr key={s.size}>
                            <td className="py-1.5 px-3 font-bold text-neutral-950">{s.size}</td>
                            <td className="py-1.5 px-3 font-bold text-neutral-900 text-right">{s.quantity.toLocaleString()} {activePreview.unit}</td>
                            <td className="py-1.5 px-3 text-right text-stone-300 font-mono italic">________________ [ {activePreview.unit} ]</td>
                            <td className="py-1.5 px-3 text-center text-stone-300">[ No / Yes ]</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td className="py-2 px-3 font-bold text-neutral-950">Flat Non-Size Allocation</td>
                          <td className="py-2 px-3 font-bold text-neutral-900 text-right">{activePreview.quantity.toLocaleString()} {activePreview.unit}</td>
                          <td className="py-2 px-3 text-right text-stone-300 font-mono italic">________________ [ {activePreview.unit} ]</td>
                          <td className="py-2 px-3 text-center text-stone-300">[ No / Yes ]</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                ...
                <div className="mt-3 text-[10.5px]">
                  <span className="block text-[8px] uppercase font-bold text-slate-400 mb-1 font-bold">Floor Manager Instructions:</span>
                  <div className="border border-amber-200 bg-amber-50/40 p-2.5 rounded text-neutral-800 italic leading-snug">
                    {activePreview.remarks || 'Maintain standard quality assurance tests, verify material edge thickness.'}
                  </div>
                </div>

                {/* Bottom Signatures */}
                <div className="grid grid-cols-3 gap-6 text-center text-[7.5px] font-bold text-neutral-500 mt-10">
                  <div className="border-t border-neutral-300 pt-1.5 font-bold">Machine Operator</div>
                  <div className="border-t border-neutral-300 pt-1.5 font-bold">Quality Inspector</div>
                  <div className="border-t border-neutral-300 pt-1.5 font-bold">Floor Unit Master</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {jobCardToPrint && (
        <>
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 overflow-y-auto print:hidden">
            <div className="bg-slate-50 border border-slate-200 rounded-2xl max-w-2xl w-full shadow-lg p-6 space-y-4 text-xs animate-fade-in text-left">
              ...
              <div className="w-full bg-white border border-slate-300 rounded-xl p-5 shadow-sm overflow-x-auto max-h-[60vh] select-text">
                <div className="p-6 border-2 border-neutral-800 rounded-xl text-left bg-white relative">
                  ...
                  <table className="w-full text-left text-[10px] mt-4 border-collapse">
                    <thead>
                      <tr className="bg-slate-100 font-bold border-b border-slate-300">
                        <th className="py-1 px-2">Size Name (সাইজ)</th>
                        <th className="py-1 px-2 text-right">Target Quantity</th>
                        <th className="py-1 px-2 text-right">Actual QC Check (Pcs)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {jobCardToPrint.sizes && jobCardToPrint.sizes.length > 0 ? (
                        jobCardToPrint.sizes.map((s, idx) => (
                          <tr key={idx}>
                            <td className="py-1.5 px-2 font-bold text-slate-700">{s.size}</td>
                            <td className="py-1.5 px-2 font-bold text-right">{s.quantity.toLocaleString()} {jobCardToPrint.unit}</td>
                            <td className="py-1.5 px-2 text-right text-stone-300">________________ [ Pcs ]</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td className="py-1.5 px-2 font-bold text-slate-700">Flat Target</td>
                          <td className="py-1.5 px-2 font-bold text-right">{jobCardToPrint.quantity.toLocaleString()} {jobCardToPrint.unit}</td>
                          <td className="py-1.5 px-2 text-right text-stone-300">________________ [ {jobCardToPrint.unit} ]</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          <div className="border-2 border-neutral-800 p-8 rounded-xl max-w-4xl mx-auto space-y-6 bg-white relative text-left">
            ...
            <div className="border border-neutral-200 rounded-lg overflow-hidden text-left">
              <div className="bg-neutral-100 px-4 py-2 font-black text-xs text-neutral-800 uppercase tracking-wider">
                Production SizeWise Allocations (সাইজ অনুযায়ী উৎপাদনের পরিমাণ)
              </div>
              <table className="w-full text-left text-xs text-neutral-700">
                <thead className="bg-neutral-50 border-b border-neutral-200 font-bold uppercase text-[9.5px] tracking-wider text-neutral-500">
                  <tr>
                    <th className="py-2 px-4">Size Name (সাইজ)</th>
                    <th className="py-2 px-4 text-right">Required Target Quantity (উৎপাদন লক্ষ্যমাত্রা)</th>
                    <th className="py-2 px-4 text-right">Actual Output Check (Pcs)</th>
                    <th className="py-2 px-4 text-center">Defect Check</th>
                  </tr>
                </thead>
                  ...
                  {jobCardToPrint.sizes && jobCardToPrint.sizes.length > 0 ? (
                    jobCardToPrint.sizes.map((s, idx) => (
                      <tr key={idx} className="hover:bg-neutral-50/50">
                        <td className="py-2 px-4 font-black text-neutral-950">{s.size}</td>
                        <td className="py-2 px-4 font-bold text-neutral-900 text-right">{s.quantity.toLocaleString()} {jobCardToPrint.unit}</td>
                        <td className="py-2 px-4 text-right text-stone-300 font-mono">________________ [ Pcs ]</td>
                        <td className="py-2 px-4 text-center text-stone-300"> [ No / Yes ]</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td className="py-3 px-4 font-black text-neutral-950">Non-Size Flat Allocation</td>
                      <td className="py-3 px-4 font-bold text-neutral-900 text-right">{jobCardToPrint.quantity.toLocaleString()} {jobCardToPrint.unit}</td>
                      <td className="py-3 px-4 text-right text-stone-300 font-mono">________________ [ {jobCardToPrint.unit} ]</td>
                      <td className="py-3 px-4 text-center text-stone-300"> [ No / Yes ]</td>
                    </tr>
                  )}
                </tbody>
              </table>
